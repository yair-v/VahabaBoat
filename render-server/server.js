...
app.post('/api/update', auth, (req, res) => {
  ...
  const item = {
    ...
    rawLow: Array.isArray(rawLow)
      ? rawLow
      : [],

    rawHigh: Array.isArray(rawHigh)
      ? rawHigh
      : [],
  ...
  const result =
    Array.from(devices.values())
      ...
      .map((d) => {
        ...
        const online =
          now - d.lastSeen <= OFFLINE_AFTER_MS;
...
app.get('/api/history/:deviceId', (req, res) => {
  ...
  const arr =
    history.get(req.params.deviceId) || [];
...
async function loadData() {
  ...
  try {
    ...
    if (!devices.length) {
      ...
      root.innerHTML =
        '<div class="empty">אין עדיין נתונים. הפעל את סקריפט ה־Raspberry Pi.</div>';
...
\`;
    ...
    document.getElementById(
      ...
      '<div class="empty">שגיאה בטעינת נתונים</div>';
  }
}
...
app.listen(PORT, () => {
  ...
  console.log(
    \`Water monitor server running on port \${PORT}\`
  );
